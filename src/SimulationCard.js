...
export class SimulationCard extends React.Component {
  ...
  render() {
    return (
      <div>
        ...
        { this.state.simulated || this.state.disabled ? <RateCard usage={this.props.usage} production={this.props.production} simulated={this.state.simulated}/> : null }
      </div>
    );
  }
  ...
  componentDidMount() {
    ...
    if(!this.state.disabled && localStorage.getItem('simulationDisabled'))
    {
      this.setState({
        disabled: true
      })
